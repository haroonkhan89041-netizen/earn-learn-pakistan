  ...
  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <div>
        ...
        <form onSubmit={onSubmit} className="card space-y-4 p-5">
          <div>
            ...
            <select className="input" value={category} onChange={(e) => setCategory(e.target.value as SupportCategory)}>
      ...
      <div>
        ...
        {loading ? (
          <div className="card p-5 text-sm text-navy-500">Loading your tickets…</div>
        ) : tickets.length === 0 ? (
          ...
          <div className="card divide-y divide-navy-100">
            {tickets.map((t) => (
              <div key={t.id} className="p-4">
                ...
                {t.admin_reply && <div className="mt-3 rounded-lg bg-navy-50 p-3 text-sm text-navy-700"><span className="font-semibold">Support reply:</span> {t.admin_reply}</div>}
                <p className="mt-1 text-xs text-navy-400">{new Date(t.created_at).toLocaleDateString()}</p>
              </div>
            ))}
          </div>
        )}
