    ...
    <div className="min-h-screen bg-navy-50/50 md:flex">
      <aside className="w-full shrink-0 border-b border-navy-100 bg-navy-900 md:h-screen md:w-64 md:border-b-0 md:border-r">
        <div className="px-5 py-5">
          <Link to="/dashboard" className="flex items-center gap-2 text-sm font-medium text-navy-300 hover:text-white"><ArrowLeft size={15} /> Back to app</Link>
        ...
        <nav className="flex flex-wrap gap-1 px-3 pb-4 md:flex-col">
          {items.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-colors ${isActive ? 'bg-white text-navy-900' : 'text-navy-300 hover:bg-navy-800 hover:text-white'}`}><item.icon size={17} /> {item.label}</NavLink>)}
        </nav>
      </aside>
      <main className="container-app flex-1 py-6"><Outlet /></main>
    </div>
  );
}
