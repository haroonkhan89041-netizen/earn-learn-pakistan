...
import {
  ...
  ChevronRight, Sparkles, LibraryBig,
} from 'lucide-react';
...
const navItems = [
  ...
  { to: '/dashboard/my-courses', label: 'My Courses', description: 'Continue learning', icon: LibraryBig, tone: 'indigo' },
      ...
      <aside className="dashboard-sidebar hidden w-[290px] shrink-0 flex-col md:flex">
        <div className="sidebar-top">
          <Link to="/" className="sidebar-brand" aria-label="Earn & Learn Pakistan home">
            ...
            <span className="sidebar-brand-copy"><span className="sidebar-brand-title">Earn &amp; Learn</span><span className="sidebar-brand-subtitle">PAKISTAN</span></span>
          ...
          <div className="sidebar-member-card">
            ...
            <div className="sidebar-member-copy"><span className="sidebar-member-eyebrow">WELCOME BACK</span><strong>{memberName}</strong><span>{user.email || 'Earn & Learn member'}</span></div>
          ...
          <nav className="sidebar-nav" aria-label="Dashboard navigation">
            {navItems.slice(0, 8).map((item) => (
              <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `dashboard-nav-item ${isActive ? 'is-active' : ''}`}>
                {({ isActive }) => <><span className={`dashboard-nav-icon tone-${item.tone}`}><item.icon size={18} strokeWidth={isActive ? 2.5 : 2} /></span><span className="dashboard-nav-copy"><span className="dashboard-nav-label">{item.label}</span><span className="dashboard-nav-description">{item.description}</span></span><span className="dashboard-nav-arrow"><ChevronRight size={15} /></span></>}
              </NavLink>
            ))}
          </nav>
          ...
          <nav className="sidebar-nav" aria-label="Account navigation">
            {navItems.slice(8).map((item) => (
              <NavLink key={item.to} to={item.to} className={({ isActive }) => `dashboard-nav-item ${isActive ? 'is-active' : ''}`}>
                {({ isActive }) => <><span className={`dashboard-nav-icon tone-${item.tone}`}><item.icon size={18} strokeWidth={isActive ? 2.5 : 2} /></span><span className="dashboard-nav-copy"><span className="dashboard-nav-label">{item.label}</span><span className="dashboard-nav-description">{item.description}</span></span>{item.badge && <span className="dashboard-nav-badge">{item.badge}</span>}<span className="dashboard-nav-arrow"><ChevronRight size={15} /></span></>}
              </NavLink>
            ))}
          </nav>
        </div>
        ...
        <div className="sidebar-bottom">
          {isAdmin && <NavLink to="/admin" className="admin-nav-item"><span className="admin-nav-icon"><ShieldCheck size={16} /></span><span><strong>Admin Panel</strong><small>Manage platform</small></span><ChevronRight size={15} /></NavLink>}
          <button onClick={signOut} className="logout-nav-item"><span className="logout-nav-icon"><LogOut size={16} /></span><span>Log out</span></button>
        </div>
      </aside>

      <div className="flex min-w-0 flex-1 flex-col">
        <header className="dashboard-mobile-header md:hidden"><Link to="/" className="flex items-center gap-2 font-display text-sm font-extrabold text-navy-900"><span className="sidebar-brand-mark mobile-brand-mark"><Logo size={15} /></span><span>Earn &amp; Learn PK</span></Link><Link to="/dashboard/profile" className="mobile-profile-button">{memberInitial}</Link></header>
        ...
        <nav className="dashboard-mobile-nav fixed inset-x-0 bottom-0 z-30 md:hidden" aria-label="Mobile dashboard navigation">
          {mobileNavItems.map((item) => <NavLink key={item.to} to={item.to} end={item.end} className={({ isActive }) => `mobile-nav-item ${isActive ? 'is-active' : ''}`}><span className={`mobile-nav-icon tone-${item.tone}`}><item.icon size={18} /></span><span>{item.label.split(' ')[0]}</span></NavLink>)}
        </nav>
      </div>
    </div>
  );
